...
export default function Home() {
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      ...
      localStorage.setItem('ks_form_details', '');
      localStorage.setItem('ks_form_username', '');

      // Store the response data in local storage
      if (data.id) {
        localStorage.setItem('ks_form_username', username);
        localStorage.setItem('ks_form_details', JSON.stringify(data));
        router.push('/form');
      }
  ...
  return (
    <main className="sm:mx-3">
      <form className="flex items-center justify-center h-screen mx-auto lg:w-1/2 md:w-1/2" onSubmit={handleFormSubmit}>
